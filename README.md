...
Skinny arrow functions are simply shorthand for the standard function keyword. They'll be translated into hoisted function declarations if assigned directly to a variable defined in the current scope. Eg. `hoistedFn = (params) -> fnBody`. Shorthand function assignment means you can replace the `= ->` when assigning an anonymous function with no parameters to an object property or variable. Eg. `hoistedFn -> fnBody`.
